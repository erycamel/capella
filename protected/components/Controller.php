...
class Controller extends CController
{
    ...
    public function ValidateData($datavalidate)
    {
      ...
      for ($row = 0; $row <  count($datavalidate); $row++)
      {
        if ($datavalidate[$row][2] == 'emptystring') {
          ...
          {
            $message = Catalogsys::model()->findbyattributes(array('catalogname'=>$datavalidate[$row][1],
              'languageid'=>Yii::app()->params['language']));
            if ($message != null) {
    ...
    public function GetMessage($catalogname,$othertext='')
    {
      ...
      if (is_array($catalogname) == false)
      {
        ...
        $message=Catalogsys::model()->findbyattributes(array('catalogname'=>(string)$catalogname,
                'languageid'=>Yii::app()->params['language']));
        if ($message != null)
        {
          $messages = $message->catalogval;
