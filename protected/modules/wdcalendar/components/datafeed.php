...
function listCalendarByRange($sd, $ed){
  ...
  try{
	...
	$sql = "select a.* ".
"from jqcalendar a ".
"inner join eventrequest b on b.eventrequestid = a.eventrequestid ".
"where b.isprivate = 0 and `starttime` between '".php2MySqlTime($sd)."' and '". php2MySqlTime($ed)."' ".
"union ".
"select a.* ".
"from jqcalendar a ".
"inner join eventrequest b on b.eventrequestid = a.eventrequestid ".
"inner join eventparticipant c on c.eventrequestid = a.eventrequestid ".
"inner join useraccess d on d.useraccessid = c.useraccessid ".
"where lower(d.username) = lower('".Yii::app()->user->id."')";
