...
class AbsscheduleController extends Controller
{
 ...
 protected $menuname = 'absschedule';
	...
	public function actionWrite()
	{
	  if(isset($_POST['Absschedule']))
	  {
        $messages = $this->ValidateData(
                array(array($_POST['Absschedule']['absschedulename'],'hrtmasemptyabsschedulename','emptystring'),
                array($_POST['Absschedule']['absin'],'hrtmasemptyabsin','emptystring'),
                array($_POST['Absschedule']['absout'],'hrtmasemptyabsout','emptystring'),
                array($_POST['Absschedule']['absstatusid'],'hrtmasemptyabsstatusid','emptystring'),
            )
        );
        if ($messages == '') {
    ...
    $pdf->Row(array('Schedule','In','Out','Status'));
    $pdf->setaligns(array('L','C','C','C'));
    foreach($dataReader as $row1)
    {
      $pdf->row(array($row1['absschedulename'],$row1['absin'],$row1['absout'],$row1['shortstat']));
    }
		...
		if (move_uploaded_file($_FILES['uploadfile']['tmp_name'], $file)) { 
		  ...
		  while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
			if ($row>1) {
			  ...
			  $model->absin = $data[2];
			  $model->absout = $data[3];
              $absstatus = Absstatus::model()->findbyattributes(array('shortstat'=>(int)$data[4]));
              if ($absstatus != null) {
