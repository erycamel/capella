	...
	parent::actionWrite();
	  if(isset($_POST['Wagetype']))
	  {
        $messages = $this->ValidateData(
                array(array($_POST['Wagetype']['wagename'],'hrmbtemptywagetypename','emptystring'),
            )
        );
        if ($messages == '') {
		if ((int)$_POST['Wagetype']['wagetypeid'] > 0)
		{
		  ...
		  $model->pphbycompany = $_POST['Wagetype']['pphbycompany'];
		  $model->paidbycompany = $_POST['Wagetype']['paidbycompany'];
		}
		else
		{
		  $model = new Wagetype();
