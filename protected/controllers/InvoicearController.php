...
protected $menuname = 'invoicear';
	  ...
	  $this->lookupdata();
		...
		if (Yii::app()->request->isAjaxRequest)
        {
            echo CJSON::encode(array(
                'status'=>'success',
				'invoiceid'=>$model->invoiceid,
					'invoicedate'=>date(Yii::app()->params['dateviewfromdb'], strtotime($model->invoicedate)),				  'amount'=>Yii::app()->numberFormatter->format(Yii::app()->params["defaultnumberprice"],$model->amount),
	...
	public function actionGeneratedetail()
          ...
          if(isset($_POST['id']))
	  {
			  ...
			  try
			  {
				$poheader = Soheader::model()->findbypk($_POST['id']);
				$docdate;
				$paymentmethodid;
				$paycode;
				$amount;
				if ($poheader !== null)
				{
					$docdate = $poheader->sodate;
					$paycode= ($poheader->paymentmethod !== null) ? $poheader->paymentmethod->paycode:"";
					$paymentmethodid= $poheader->paymentmethodid;
				}
				$sql = 'call GenerateINVSO(:vid, :vhid)';
				$command=$connection->createCommand($sql);
				$command->bindvalue(':vid',$_POST['id'],PDO::PARAM_INT);
				$command->bindvalue(':vhid', $_POST['hid'],PDO::PARAM_INT);
				$command->execute();
				$sql = 'select sum(price * qty * currencyrate) from sodetail where soheaderid = '.$_POST['id'];
				$command=$connection->createCommand($sql);
				$amount=Yii::app()->numberFormatter->format(Yii::app()->params["defaultnumberprice"],$command->queryScalar());
				$transaction->commit();
				if (Yii::app()->request->isAjaxRequest)
				{
					echo CJSON::encode(array(
	...
	public function actionWrite()
	{
	  if(isset($_POST['Invoice']))
	  {
         $messages = $this->ValidateData(
                array(
				array($_POST['Invoice']['soheaderid'],'emptyso','emptystring'),
				array($_POST['Invoice']['invoicedate'],'emptydate','emptystring'),
		...
		if ((int)$_POST['Invoice']['invoiceid'] > 0)
		{
		  ...
		  $model->soheaderid= $_POST['Invoice']['soheaderid'];
		...
		//$model->addressid = $_POST['Invoice']['addressid'];
		}
		else
		{
		  $model = new Invoice();
	...
	public function actionWriteinvoicedet()
	{
	  ...
	  $messages = $this->ValidateData(
                ...
                array($_POST['Invoicedet']['price'],'emptyprice','emptystring'),
                array($_POST['Invoicedet']['qty'],'emptyqty','emptystring'),
                array($_POST['Invoicedet']['currencyid'],'emptycurrency','emptystring'),
                array($_POST['Invoicedet']['unitofmeasureid'],'emptyunitofmeasure','emptystring'),
                array($_POST['Invoicedet']['rate'],'emptyrate','emptystring'),
            )
        );
         if ($messages == '') {
	...
	public function actionDownload()
	{
	  parent::actionDownload();
	   ...
	   (select addressname from address e where e.addressbookid = d.addressbookid order by e.addressid desc limit 1) as addressname,
