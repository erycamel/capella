...
protected $menuname = 'cbin';
	...
	public function actionCreate()
	{
	  ...
	  $this->lookupdata();
		...
		$model->recordstatus=Wfgroup::model()->findstatusbyuser('inscbin');
		...
		if (Yii::app()->request->isAjaxRequest)
        {
            echo CJSON::encode(array(
				...
				'cashbankid'=>$model->cashbankid,
				  ...
				  'currencyrate'=>Yii::app()->numberFormatter->format(Yii::app()->params["defaultnumberprice"],$model->currencyrate),
	  ...
	  $cashbankacc=$this->loadModeldetailcashbankacc($id[0]);
		...
		if (Yii::app()->request->isAjaxRequest)
        {
            echo CJSON::encode(array(
                ...
                'debit'=>Yii::app()->numberFormatter->format(Yii::app()->params["defaultnumberprice"],$cashbankacc->debit),
                'credit'=>Yii::app()->numberFormatter->format(Yii::app()->params["defaultnumberprice"],$cashbankacc->credit),
                'currencyrate'=>Yii::app()->numberFormatter->format(Yii::app()->params["defaultnumberprice"],$cashbankacc->currencyrate),
	...
	public function actionDownload()
	{
	  parent::actionDownload();
	   $sql = "select a.cashbankid, a.cashbankno, a.amount, a.accountid, b.accountname, a.currencyrate, c.symbol,a.transdate,
		a.description,e.sono,b.accountcode,f.fullname
		from cashbank a
    ...
    foreach($dataReader as $row)
    {
		...
		{
	  $this->pdf->title= 'Cash Receipt';
		}
		else 
		if ((substr($row['accountcode'],0,6) == '1.1.03') or (substr($row['accountcode'],0,6) == '1.1.04'))
		{
		$this->pdf->title='Bank Receipt';
		}
	  $this->pdf->AddPage('P');
	  // definisi font
	  $this->pdf->setFont('Arial','B',8);
		$this->pdf->rect(10,60,195,30);
		$this->pdf->setFont('Arial','B',10);
		$this->pdf->text(20,$this->pdf->gety()+5,'BR No: '.$row['cashbankno']);
		$this->pdf->text(20,$this->pdf->gety()+12,'SO No: '.$row['sono']);
		$this->pdf->text(20,$this->pdf->gety()+19,'Customer: '.$row['fullname']);
...
where cashbankid = ".$row['cashbankid'];
      ...
      $this->pdf->setwidths(array(20,50,22,23,20,15,45));
      $this->pdf->setbordercell(array('LTRB','LTRB','LTRB','LTRB','LTRB','LTRB','LTRB','LTRB'));
	  $this->pdf->colheader = array('Account No','Account Name','Debit','Credit','Amount','Rate','Description');
      $this->pdf->RowHeader();
		$this->pdf->setFont('Arial','',7);
      $this->pdf->setbordercell(array('LTRB','LTRB','LTRB','LTRB','LTRB','LTRB','LTRB','LTRB'));
      $this->pdf->coldetailalign = array('L','L','R','R','R','R','L');
	  $totaldebet = 0;
	  $totalcredit = 0;$symbol = '';
      ...
      $this->pdf->setaligns(array('L','L','R','R','L','L','R','L'));
        $this->pdf->row(array('Total','',
			Yii::app()->numberFormatter->formatCurrency($totaldebet,$row['symbol']),
			Yii::app()->numberFormatter->formatCurrency($totalcredit,$row['symbol']),
			'',
			'',
			'',
			));
