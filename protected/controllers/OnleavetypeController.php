	...
	public function actionWrite()
	{
	  ...
	  if(isset($_POST['Onleavetype']))
	  {
        $messages = $this->ValidateData(
                array(
                array($_POST['Onleavetype']['onleavename'],'hpaoltemptyonleavename','emptystring'),
                array($_POST['Onleavetype']['cutimax'],'hpaoltemptycutimax','emptystring'),
                array($_POST['Onleavetype']['cutistart'],'hpaoltemptycustistart','emptystring'),
                array($_POST['Onleavetype']['snroid'],'hpaoltemptysnroid','emptystring'),
                array($_POST['Onleavetype']['absstatusid'],'hpaoltemptyabstatusid','emptystring'),
            )
        );
        if ($messages == '') {
  ...
  public function actionDownload()
  {
    ...
    $sql = "select a.onleavename, a.cutimax,a.cutistart,b.formatdoc,c.shortstat
      ...
      left join absstatus c on c.absstatusid = a.absstatusid ";
		if ($_GET['id'] !== '') {
				$sql = $sql . "where a.onleavetypeid = ".$_GET['id'];
		}
		$command=$this->connection->createCommand($sql);
		$dataReader=$command->queryAll();
	  $this->pdf->title='Onleave Type List';
	  $this->pdf->AddPage('P');
    ...
    $this->pdf->setwidths(array(50,20,20,50,20));
    $this->pdf->Row(array('Onleave','Cuti Max','Cuti Start','Format Doc','Cuti Status'));
    $this->pdf->setaligns(array('L','L','L','L','L'));
    foreach($dataReader as $row1)
    {
      $this->pdf->row(array($row1['onleavename'],$row1['cutimax'],$row1['cutistart'],
