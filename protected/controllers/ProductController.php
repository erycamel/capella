	...
	public function actionWrite()
	{
	  ...
	  if(isset($_POST['Product']))
	  {
        $messages = $this->ValidateData(
                array(array($_POST['Product']['productname'],'mmmdemptyproductname','emptystring'),
            )
        );
        if ($messages == '') {
		...
		try
          {
            if($model->save())
            {
              ...
              $this->GetSMessage('mmmginsertsuccess');
            }
            else
            {
		...
		if (move_uploaded_file($_FILES['uploadfile']['tmp_name'], $file)) { 
			...
			if (($handle = fopen($file, "r")) !== FALSE) {
				while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
					if ($row>1) {
			  $model=Product::model()->findByPk((int)$data[0]);
			  if ($model=== null) {
				$model = new Product();
			  }
			  $model->productid = (int)$data[0];
			  $model->productname = $data[1];
			  $model->productpic = $data[2];
