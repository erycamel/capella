...
protected $menuname = 'reportperday';
    ...
    {
      if (isset($_POST['id']) & isset($_POST['statusinid']) & isset($_POST['statusoutid']) & isset($_POST['reason']))
      {
        ...
        foreach($ids as $id)
        {
          try
          {
            $sql = 'call fixabstrans(:vid, :vinid ,:voutid, :vreason)';
            $command=$connection->createCommand($sql);
            $command->bindValue(':vid',$id,PDO::PARAM_INT);
            $command->bindValue(':vinid',$_POST['statusinid'],PDO::PARAM_INT);
            $command->bindValue(':voutid',$_POST['statusoutid'],PDO::PARAM_INT);
         ...
         echo CJSON::encode(array(
                'status'=>'failure',
                'div'=>$this->GetSMessage('htrpderrorstatusin')
				));
            Yii::app()->end();
      }
    }
	...
	{
	  parent::actionIndex();
    $model=new Reportperday('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Reportperday']))
			$model->attributes=$_GET['Reportperday'];
if (isset($_GET['pageSize']))
	...
	public function actionDownload()
  {
    ...
    $model=new reportperday('searchwstatus');
  ...
  public function actionUpload()
	...
	{
		$model=Reportperday::model()->findByPk((int)$id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='reportperday-form')
		{
