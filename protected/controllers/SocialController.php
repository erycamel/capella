...
protected $menuname = 'social';
    ...
    {
      $this->DeleteLockCloseForm($this->menuname, $_POST['Social'], $_POST['Social']['socialid']);
    }
	...
	public function actionWrite()
	{
	  ...
	  if(isset($_POST['Social']))
	  {
        $messages = $this->ValidateData(
                array(array($_POST['Social']['socialname'],'hmsoemptysocialname','emptystring'),
            )
        );
        if ($messages == '') {
		//$dataku->attributes=$_POST['Social'];
		if ((int)$_POST['Social']['socialid'] > 0)
		{
		  $model=$this->loadModel($_POST['Social']['socialid']);
		  $model->socialname = $_POST['Social']['socialname'];
		  $model->recordstatus = $_POST['Social']['recordstatus'];
		}
		else
		{
		  $model = new Social();
		...
		try
          {
            if($model->save())
            {
              $this->DeleteLock($this->menuname, $_POST['Social']['socialid']);
              $this->GetSMessage('hmsoinsertsuccess');
            }
            else
            {
...
if (isset($_GET['pageSize']))
	...
	public function actionUpload()
	{
	  ...
	  if (($handle = fopen($folder.$uploader->file->getName(), "r")) !== FALSE) {
		  while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
			if ($row>0) {
			  $model=Social::model()->findByPk((int)$data[0]);
			  if ($model=== null) {
				$model = new Social();
			  }
			  $model->socialid = (int)$data[0];
			  $model->socialname = $data[1];
	...
	public function actionDownload()
  {
    ...
    // menuliskan tabel
    $header = array('No','ID','Social Name');
    $model=new Social('search');
