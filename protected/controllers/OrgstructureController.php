...
if ($model != null)
	...
	public function actionWrite()
	{
	  ...
	  if(isset($_POST['Orgstructure']))
	  {
        $messages = $this->ValidateData(
                array(array($_POST['Orgstructure']['structurename'],'hoosemptystructurename','emptystring'),
            )
        );
        if ($messages == '') {
		...
		if (move_uploaded_file($_FILES['uploadfile']['tmp_name'], $file)) { 
		  ...
		  while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
			if ($row>0) {
			  ...
			  $orgstructure = Orgstructure::model()->findbyattributes(array('structurename'=>$data[2]));
			  if ($orgstructure !== null)
			  {
				$model->parentid = $orgstructure->orgstructureid;
				}
	...
	public function actionDownload()
  {
    ...
    $this->pdf->Output('orgstructure.pdf','D');
  }
