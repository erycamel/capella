...
if ($model != null)
	...
	public function actionWrite()
	{
	  if(isset($_POST['Absrule']))
	  {
        $messages = $this->ValidateData(
                array(array($_POST['Absrule']['absscheduleid'],'hrtmaremptyabsscheduleid','emptystring'),
                array($_POST['Absrule']['difftimein'],'hrtmaremptydifftimein','emptystring'),
                array($_POST['Absrule']['difftimeout'],'hrtmaremptydifftimeout','emptystring'),
                array($_POST['Absrule']['absstatusid'],'hrtmaremptyabsstatusid','emptystring'),
            )
        );
        if ($messages == '') {
  ...
  public function actionDownload()
  {
    ...
    $pdf->Row(array('Schedule','Time In','Time Out','Status'));
    $pdf->setaligns(array('C','C','C','C'));
    foreach($dataReader as $row1)
    {
      $pdf->row(array($row1['absschedulename'],$row1['difftimein'],
